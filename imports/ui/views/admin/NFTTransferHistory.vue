<template>
    ...
    <div>
        ...
        <div class="title-group">
            <div class="main-title">{{$t('admin.landTransferHistory')}}</div>
        </div>
        ...
        <el-table :data="dataList" style="width:100%;margin-top: 20px;" border fit highlight-current-row
            ...
            <el-table-column :label="$t('admin.transferingNFT')" min-width="100" align="center">
                <template slot-scope="{row}">
                    <!-- <div>{{row.userData ? (row.userData.idOnChain || '') : ''}}</div> -->
                    <a @click="showNFTDetail(row.nftInfo[0])">{{showNFT(row)}}</a>
                </template>
            </el-table-column>
        ...
        <el-dialog :title="$t('admin.transferNFT')" :visible.sync="showTransferNFT" append-to-body class="dialog-add" width="500px">
            <el-form label-width="150px" status-icon>
                ...
                <el-form-item :label="$t('admin.transferToUser')">
                    <el-input v-model="transferToUser" :placeholder="$t('admin.inputUsername')"></el-input>
                </el-form-item>
    ...
    export default {
        ...
        data() {
            return {
                ...
                allTypes: Object.keys(NonFungibleTokenChangeType),
                allTypeValueKeys: {},
        ...
        methods: {
            ...
            convertType(type) {
                if (type === 1) {
                    return this.$t('admin.casting')
                } else if (type === 2) {
                    return this.$t('my.buyHistory.deposit')
                } else if (type === 3) {
                    return this.$t('my.buyHistory.withdraw')
                } else if (type === 4) {
                    return this.$t('my.balanceRecord.withdrawCancel')
                } else if (type === 5) {
                    return this.$t('admin.transferOut')
                } else if (type === 6) {
                    return this.$t('admin.transferIn')
                } else if (type === 7) {
                    return this.$t('admin.stake')
                } else if (type === 8) {
                    return this.$t('admin.redeem')
                }
                return ''
            },
            convertedAllTypes() {
            ...
            showNFT(item) {
                if (item.nftInfo && (item.nftInfo.length || 0) > 0) {
                    const nft = item.nftInfo[0]
                    return nft.state.idOnChain || ''
                }
                return ''
            },
            _checkRoles() {
            ...
            _fetchData() {
                ...
                if (this.selectedType) {
                    params.type = this.selectedType + 1
                }
                const self = this;
                this.isLoading = true

                Meteor.call("admin_getMoveNftHistory", params, (err, res) => {
